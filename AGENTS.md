...
**Email templates restyled to owner example (D-0180)** + Falcon domain presence + OG share defaults (D-0179) + Admin Platform (D-0176–D-0178) — shared branded email chrome (navy header + falcon lockup, feature pillars, approved mission quote, contact/footer); approved falcon brand mark on domain/hub mastheads; sitewide Open Graph / Twitter cards use logo + **SAVEN Core** (`/brand/og-default.png`, `metadataBase` https://www.savencore.com); restricted `/[locale]/admin/` with branded email templates, roles/permissions/users/invitations/notifications/mailings (JSON store; SMTP optional / simulated); demo/owner Credentials operator always `super_admin`; public Experience Redesign UI freeze (D-0175) unchanged for heroes — read `docs/ADMIN_PLATFORM.md`, `docs/SITE_ASSIGNMENT.md`, `docs/SITE_AUDIT_REPORT.md`, and D-0134–D-0180.
