...
**Robotics Interface diagram brand mark + photoreal thumbs (D-0191)** + diagram enrichment (D-0190) + SAVEN Robotics Interface content + hub diagram (D-0189) + Footer Architecture column (D-0188) + Admin Media filter chip contrast (D-0187) + delete-all + upload CTAs + public Media viewer (D-0186) + traditional CMS UX (D-0185) + simplified upload (D-0184) + Email templates + Media library (D-0183) + Home Application rename (D-0182) + Footer Legal equal columns + More hub + socials bottom row (D-0181) + Email chrome (D-0180) + Falcon/OG (D-0179) + Admin Platform (D-0176–D-0178) — Robotics Interface diagram: **SAVEN logo mark** in center node; circular photoreal device thumbs from approved crops (`public/domain/systems/diagram/`); per-device color accents + motion / link traffic from D-0190 (respects reduced-motion); page teaches shared communication/control under SAVEN (In Development); Footer **Architecture** column lists all published Systems pages (Overview + leaves + Robotics Interface; routes `/systems/*`); Admin Media library filters **Library / Videos / Docs / Links** with clear light/dark contrast; **Delete on every row** (seed soft-hide; upload/link hard-delete); navy/gold elevated **Upload** CTAs; public `/[locale]/media/` viewer gallery (filters Videos/Docs/Links, View + Download cards); traditional CMS tabs Upload file / Upload video / Add link; YouTube/Vimeo CSP embeds; honest Vercel storage limits; Media hub in footer Company; expanded business-life email templates (D-0180 chrome); Applications leaf **Home Application**; footer Legal equal columns + **More** → `/legal/`; socials under column grid; branded email chrome; falcon on domain mastheads; OG defaults (`/brand/og-default.png`); restricted `/[locale]/admin/`; demo operator always `super_admin`; Experience Redesign UI freeze (D-0175) unchanged for heroes — read `docs/ADMIN_PLATFORM.md`, `docs/SITE_ASSIGNMENT.md`, `docs/SITE_AUDIT_REPORT.md`, and D-0134–D-0191.
