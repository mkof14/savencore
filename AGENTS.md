...
**Robotics Interface diagram large SVG device illustrations (D-0193)** + section polish (D-0192) + brand mark (D-0191) + diagram enrichment (D-0190) + SAVEN Robotics Interface content + hub diagram (D-0189) + Footer Architecture column (D-0188) + Admin Media filter chip contrast (D-0187) + delete-all + upload CTAs + public Media viewer (D-0186) + traditional CMS UX (D-0185) + simplified upload (D-0184) + Email templates + Media library (D-0183) + Home Application rename (D-0182) + Footer Legal equal columns + More hub + socials bottom row (D-0181) + Email chrome (D-0180) + Falcon/OG (D-0179) + Admin Platform (D-0176–D-0178) — Robotics Interface diagram: **SAVEN logo mark** at center; **large high-contrast SVG device illustrations** (not photo circles) with clear labels and color-coded links; large **SAVEN Robotics Interface** section title; navy/gold section atmosphere; motion from D-0190 (respects reduced-motion); page teaches shared communication/control under SAVEN (In Development); Footer **Architecture** column lists all published Systems pages (Overview + leaves + Robotics Interface; routes `/systems/*`); Admin Media library filters **Library / Videos / Docs / Links** with clear light/dark contrast; **Delete on every row** (seed soft-hide; upload/link hard-delete); navy/gold elevated **Upload** CTAs; public `/[locale]/media/` viewer gallery (filters Videos/Docs/Links, View + Download cards); traditional CMS tabs Upload file / Upload video / Add link; YouTube/Vimeo CSP embeds; honest Vercel storage limits; Media hub in footer Company; expanded business-life email templates (D-0180 chrome); Applications leaf **Home Application**; footer Legal equal columns + **More** → `/legal/`; socials under column grid; branded email chrome; falcon on domain mastheads; OG defaults (`/brand/og-default.png`); restricted `/[locale]/admin/`; demo operator always `super_admin`; Experience Redesign UI freeze (D-0175) unchanged for heroes — read `docs/ADMIN_PLATFORM.md`, `docs/SITE_ASSIGNMENT.md`, `docs/SITE_AUDIT_REPORT.md`, and D-0134–D-0193.

Hero: **Intelligence for the Physical World.** + living photoreal **help collage** (D-0149/D-0151/D-0152). Post-hero: **one** care-focused living carousel (D-0150). **Page bodies + UI chrome for all 10 locales** (`en`/`es`/`de`/`fr`/`ja`/`zh-cn`/`ar`/`he`/`uk`/`ru`; English canonical). Header: logo + **important Layer-1 hubs** (≤7) + Sign In/Up text link + sun/moon theme + language with flags (D-0153/D-0155/D-0156; Install app is footer-only — D-0164). Hubs **and domain leaves** share one visual shell (`DomainVisualPage` / Layer-1 hub grammar) — thematic masthead, short human lead, editorial story-band cues (not bordered WHAT/WHY/NEXT boxes), progressive depth (D-0157–D-0160; opening restyle D-0165). Thematic WebP mastheads under `public/domain/` (+ reused care/home assets where perfect). **Robotics Lab / Labs / Investors** brochure depth expanded honestly (D-0159; no fake traction/returns). Footer: full depth map **including Architecture** (Systems published leaves under that column title — D-0188), **Legal** (primary links + More → `/legal/` hub; draft pages — pending legal review; D-0154 / D-0181) and **Media** in Company → `/media/` (D-0183), social icons under the column grid, Copyright line, Install app, upward language menu with flags, theme icons. Auth: `/[locale]/auth/sign-in/` substantial SAVEN card — email/password (env operator allowlist) + show/hide password + Google; public soft-degrade when unset — no env/setup panels (D-0156/D-0158/D-0163/D-0164; no fake logged-in state; full user DB later). Brand text: `BrandName` matches logo wordmark sitewide (D-0164). Tagline: Turning Intelligence Into Human Care. Deploy target: Vercel (App Router); see `docs/VERCEL_DEPLOY.md`.
