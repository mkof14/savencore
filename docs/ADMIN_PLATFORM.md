...
8. **Media library** (D-0183 / D-0184 / D-0185) — seed brand assets + curated site links + local upload/link store at `storage/admin-media/` (dev). **Traditional CMS UX:** tabs **Upload file | Upload video | Add link**; visible Choose file / Choose video inputs; upload progress; Video URL form with YouTube/Vimeo embed preview (CSP `frame-src` / `img-src` allowlisted); library table (Name / Type / Date / Open · Copy · **Delete**) with All files · Videos · Links filters. Mutations: editor+ and granular `manage_media`. Seed / `seed-*` rows are not deletable. Clear errors for size (40 MB local; ~4.5 MB Vercel body), type, and storage. On Vercel, filesystem mutations are blocked honestly (banner) — add media locally; large videos should use URL embeds once durable object storage ships. Public `/[locale]/media/`; public file route `/api/media/[id]/`.
