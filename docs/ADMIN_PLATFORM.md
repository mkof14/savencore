...
8. **Media library** (D-0183) — seed brand assets + curated site links + local upload/link store at `storage/admin-media/` (dev). Upload: image/video/document/presentation (pdf/doc/docx/ppt/pptx/key). Link entries: URL + title + type. Filter by category; preview / download / copy / share / delete (seed assets protected; mutations editor+). Public items surface on `/[locale]/media/`; public file route `/api/media/[id]/`. **Honest note:** Vercel filesystem is not durable — uploads/index writes may be unavailable or ephemeral until object storage.
