...
8. **Media library** (D-0183 / D-0184) — seed brand assets + curated site links + local upload/link store at `storage/admin-media/` (dev). **Add content** UX: File | Video | Link tabs; drag-and-drop / browse (pdf/doc/docx/ppt/pptx/key/mp4/webm/images); Video = YouTube/Vimeo/direct URL with live embed preview or file upload; Link = title + URL + optional note. After add: success strip with Copy link / Open / Delete. Filter by category; card list with preview. Mutations: editor+ (`media_upload` / `manage_media`). Public items on `/[locale]/media/` (YouTube/Vimeo embed in preview); public file route `/api/media/[id]/`. Storage durability note is secondary in the UI — Vercel filesystem is not durable until object storage.
