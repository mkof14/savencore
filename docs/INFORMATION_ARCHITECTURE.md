...
**As-built visitor footer (D-0132 / D-0188 / D-0205 / D-0206 / D-0202):** published destinations only, grouped by domain — Technology, **Architecture** (Systems domain depth map: Overview + published `/systems/*` leaves including SAVEN Robotics Interface; routes unchanged), Applications, Trust, Research, Labs, Company (About → `/foundation/`, Mission → `/purpose/`, Media → `/media/`), Resources (FAQ → `/faq/`, Report a Security Issue), Legal (primary links + More → `/legal/`). Desktop: **five equal columns** with stacked section groups (Technology+Architecture · Labs+Applications · Trust+Research · Company+Resources · Legal) — D-0205; spacing/type refine D-0206; supersedes D-0199 / D-0200. Full technical leaf lists live here as the Layer 2 depth map; primary header stays short. No Coming Soon badges in the visitor footer. Target taxonomy below remains the longer-term IA inventory; do not invent unpublished leaves as live links.
