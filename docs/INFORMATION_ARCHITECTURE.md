...
**As-built (D-0247):** the public Research hub and leaves (`/research/`, `/research/areas/`, `/research/notes/`) are **unpublished**. Use Applications → Research Applications (`/applications/research-applications/`), Roadmap Direction, and Legal → Research Disclaimer for public research-oriented framing. Status vocabulary **Research** remains for development honesty.

Historical IA inventory (not live routes):
...
**As-built visitor footer (D-0132 / D-0188 / D-0205–D-0213 / D-0202 / D-0216 / D-0247):** published destinations only, grouped by domain — Technology, **Architecture** (Systems domain depth map: Overview + published `/systems/*` leaves including Robotics Interface (footer label; full name on page); routes unchanged), Applications, Trust, Labs, Company (**Home** → `/`, About → `/foundation/`, Mission → `/purpose/`, Media → `/media/`), Resources (FAQ → `/faq/`, Search → `/search/`, Report a Security Issue; Install app PWA control always visible under Resources — D-0213 / D-0223 / D-0224; same type as column links — D-0223; How to install panel when no native prompt — D-0224), Legal (primary links + More → `/legal/`). **Research footer column removed (D-0247).** Desktop: **eight equal columns in one forced row** (Technology · Architecture · Labs · Applications · Trust · Company · Resources · Legal) — D-0209 pattern retained, column count updated by D-0247; supersedes D-0205 stacked five-column map; accordion below 860px; D-0208 compress labels / Robotics Interface footer label remain; D-0210 footer language hover accent; D-0211 larger links + wider gaps + medical disclaimer; **D-0212** footer form removed (Contact only), disclaimer between socials and copyright, denser link lists, three-line slogan lockup; **D-0213** Install app relocated from bottom utilities into Resources (smaller type); Admin Platform follows site light/dark + logo. Full technical leaf lists live here as the Layer 2 depth map; primary header stays short (Home first-class; Trust footer-only — D-0247). No Coming Soon badges in the visitor footer. Target taxonomy below remains the longer-term IA inventory; do not invent unpublished leaves as live links.
...
**As-built clarity pack (D-0219 / D-0220 / D-0221 / D-0225 / D-0226 / D-0227 / D-0228 / D-0229 / D-0230 / D-0231 / D-0234 / D-0235 / D-0236 / D-0237 / D-0238 / D-0239):** When `HOME_CLARITY_V1` is on (default), post-hero home adds definition, 3-step chain, compact Explore SAVEN letter showcase (logo + larger S–A–V–E–N tabs; glow contained to that band), audience fork, care-stage why-line; after carousel: **one merged BioMath bridge** (Who we are / BioMath Core → SAVEN / Human Data Model continuous context / 20 categories · 200+ services; smaller transparent BMC logo; CTA to `/foundation/biomath-core/`), and “what we are not”; rich closing Explore band remains (rail shows letter + pillar). Foundation BioMath Core layer carries the 20-category scope grid + 200+ services highlight (Architecture / In Development); deep page at `/foundation/biomath-core/` (D-0228–D-0239: **unified English owner-grade illustration panels** in themeable chrome with harmonious smaller max-widths (~40rem / Black Box ~34rem); Black Box sensitive-storage architecture intent + soft links to published Trust/Privacy / Trust/Security / Privacy Policy; light on-page TOC; Complete Services Catalog + **20-category themeable artboard panels** — Environments-like chrome, larger titles, **no per-card service counts**; Continue exploring includes Trust paths). FAQ includes dedicated BioMath coverage and reports→actions Q&As (cross-linked to the leaf). When `HOME_CLARITY_V2` is also on (default), other clarity cards stay compact with richer navy/gold/off-white grammar (D-0221). Revert density: `NEXT_PUBLIC_HOME_CLARITY_V2=false`. Revert full clarity: `NEXT_PUBLIC_HOME_CLARITY_V1=false` (see `src/config/home-clarity.ts` / D-0219 / D-0220 / D-0221 / D-0225 / D-0226 / D-0227 / D-0228).
