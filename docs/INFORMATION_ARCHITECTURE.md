...
**As-built visitor footer (D-0132 / D-0188 / D-0200):** published destinations only, grouped by domain — Technology, **Architecture** (Systems domain depth map: Overview + published `/systems/*` leaves including SAVEN Robotics Interface; routes unchanged), Applications, Trust, Research, Labs, Company (About → `/foundation/`, Mission → `/purpose/`, Media → `/media/`), Resources, Legal (primary links + More → `/legal/`). Desktop: two balanced section-column rows (5 + 4) with visual separations (D-0200; supersedes D-0199). Full technical leaf lists live here as the Layer 2 depth map; primary header stays short. No Coming Soon badges in the visitor footer. Target taxonomy below remains the longer-term IA inventory; do not invent unpublished leaves as live links.
