...
1. **Particle morph stage** (D-0255 / D-0258) — one full-bleed **16:9 cinematic frame at a time** via WebGL2 (650000 points × 4 professional scenes from approved HTML: HUMAN → INTERFACE → ROBOT → WATER; ~24s snappy loop; dense additive particles; never the vertical 5-panel reference sheet; single-frame load poster; reduced-motion poster)
