...
6. Footer visitor links are **published destinations only** — no Coming Soon badges in the public footer. Legal column restored with interim policies (D-0216; supersedes structural-DRAFT banner of D-0154).
...
**As-built footer (D-0181–D-0213 / D-0202 / D-0247):** Technology · **Architecture** (Systems leaves + SAVEN Robotics Interface; routes stay `/systems/*`) · Labs · Applications · Trust · Company (**Home** → `/`, About → `/foundation/`, **BioMath Core → `/foundation/biomath-core/`**, Mission → `/purpose/`, Investors, Investor Contact, Media → `/media/`, Contact → `/contact/`, Roadmap → `/roadmap/`) · Resources (FAQ → `/faq/`, Search → `/search/`, Report a Security Issue; **Install app** PWA control always visible under this column — D-0213 / D-0223 / D-0224; **same type as column links** (`0.6875rem` / `0.75rem`) — D-0223; native prompt or How to install panel with Close inside panel — D-0224; supersedes D-0222 shrink) · Legal (primary links + More → `/legal/` hub; interim owner-authorized policies — D-0216; counsel review still recommended). Desktop layout: **eight equal columns** — Technology · Architecture · Labs · Applications · Trust · Company · Resources · Legal (D-0247; Research hub column removed). Care stage Application CTAs on large slides (D-0209). Footer language/theme utility hover accent (D-0210). D-0211: larger footer link type + wider column gaps; protective medical disclaimer; navy-tile favicon `?v=211`. **D-0212:** Get in touch form removed from footer (Contact page only); medical disclaimer between socials and copyright bar; denser link-list rhythm; three-line slogan navy/gold lockup. **D-0213:** Admin follows site light/dark + logo + richer accents; Install app relocated from bottom utilities bar into Resources. D-0208 socials/bar/carousel thumbs + Robotics Interface footer label remain. Theme chrome (D-0207/D-0211): light cool off-white (`#e6e6e9` / `#eeeeef`); dark soft gray-blue field (`#1c1f26` / `#252830`) — not pure white / deep template navy. Header hubs (≤7): **Home**, Labs, **Systems**, Applications, Technology, Investors (D-0247; Trust footer-only; Research hub removed). **D-0248:** Investors brochure polish + Contact/Investor Contact subject alignment + Investors locale dictionary audit.
...
### Homepage region order (as-built, D-0135 / D-0150 / D-0194 / D-0219 / D-0255)

1. **Hero** (D-0261) — brand copy + **living photoreal help collage** (`HeroLivingMedia`). Particle morph retired (D-0267). Lab hub `/lab/` shows a **multi-clip** light dual-format video splash **embedded into the page surface** with a compact playlist switcher (D-0278 — **Site** default + GWR + SAVEN; extensible) and **symmetric** soft feathered edges (D-0266 / D-0267 / D-0268 / D-0269 / D-0270 / D-0271 / D-0272 / D-0273 / D-0274 / D-0275 / D-0276 / D-0277 / D-0278 / D-0279 — Site from master `SAVEN_site 1` as first/main; GWR opening dots-person from master t=0 restored (D-0276); SAVEN S-2 mark cut; **original 16:9 framing** without center zoom-crop; clear picture without grain/heavy vignette/grade; equal top/bottom + left/right page-background blend; tasteful text overlay + glass-light links to published routes; **clickable chapter ticks** + **timed captions** Understanding → Assistance → Care per active clip; **mobile: interactive controls below the video stage** so buttons do not cover the picture (D-0279; desktop keeps on-stage chrome); light side ambient; scroll-linked caption fade; optional sound/mute; light parallax + softened cursor; gentle Ken Burns; no hard video box or editorial frame; depth blur duplicate removed; not on public home).
2. **Hero copy** — brand, hero line, one breath sentence, three build pillars, tagline, footer cue (beside collage)
3. **Clarity pack** (D-0219 / D-0220 / D-0221 / D-0225 / D-0226 / D-0227 / D-0228 / D-0229 / D-0230 / D-0231 / D-0232 / D-0233 / D-0234 / D-0235 / D-0236 / D-0237 / D-0238 / D-0239 / D-0243; flags `HOME_CLARITY_V1` + `HOME_CLARITY_V2`, default on) — visual What is SAVEN · 3-step chain · **Explore SAVEN** compact cinematic letter showcase (logo + larger S–A–V–E–N tabs) · audience fork; after carousel: **one merged BioMath bridge** (Who we are / BioMath Core → SAVEN / Human Data Model continuous context / 20 categories · 200+ services · Architecture / In Development · not Operational; smaller transparent BMC mark; CTA → `/foundation/biomath-core/`) · what we are not. BioMath Core leaf (D-0239 / D-0243): unified English owner-grade illustration series (cache-busted Engine EN), **harmonious smaller diagram panels** (~40rem / Black Box ~34rem; mobile tighter) with tighter rhythm, sharper HUD hero band + **Environments-like themeable category artboard panels** (larger titles; **no per-card service counts**) + Black Box **sensitive personal data storage / protection architecture intent** with soft cross-links to published `/trust/privacy/`, `/trust/security/`, and `/legal/privacy-policy/` (not Operational guarantees) + concise **What BioMath Core is not** + light on-page TOC + Trust paths in Continue exploring; Contact soft IP rate-limit with mailto degrade (D-0243); supersedes plain CSS count cards of D-0232 for category presentation; refines D-0234–D-0238. Set `HOME_CLARITY_V1` false to restore pre-D-0219 order; set `HOME_CLARITY_V2` false to keep V1 blocks with pre-density layout.
4. **Care living carousel** — one care-focused living band (hospitals / home / family / emergency / …); with clarity on: stage “Why this is SAVEN” + Application CTA
5. **Flagship Gateway** — compact paths: Robotics Lab, Robotics Interface, Internal Future Lab (+ optional Investors) with honest statuses
6. **Closing** — SAVEN meaning band: Explore pillars (D-0216) + wordmark glow (~2× D-0218) + dense always-visible upper corner nav (Purpose · Labs · tech leaves | systems · Future Lab · Trust · Contact · FAQ — D-0217/D-0218/D-0221); dark artboard outer frame matched to YouTube feature below (`--pw-media-frame-max: 72rem`, shared shell/border — D-0249)
7. **YouTube feature** — strong presentation band after Explore map (D-0245); frame width/chrome paired with closing artboard (D-0249)
8. **Footer** — complete published site map by domain
...
**Naming (D-0215):** Internal Future Lab is a **deliberate new name** that replaces an outdated understanding of classic **R&D**. SAVEN believes traditional research-and-development framing (siloed projects, product-shaped pipelines) is **not enough** for continuous, embodied inquiry into how intelligence might carefully support people in the physical world. Public status remains **Research** — not Operational product claims.
