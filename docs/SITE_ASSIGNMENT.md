...
**As-built footer (D-0181–D-0199):** Technology · **Architecture** (Systems leaves + SAVEN Robotics Interface; routes stay `/systems/*`) · Labs · Applications · Trust · Research (Overview + Areas + Notes) · Company (About → `/foundation/`, Mission → `/purpose/`, Investors, Investor Contact, Media → `/media/`, Contact → `/contact/`, Roadmap → `/roadmap/`) · Resources (Report a Security Issue) · Legal (primary links + More → `/legal/` hub; draft framing). Desktop layout: **nine equal columns in one row** (D-0199; mid-width 3×3 wrap). Header hubs (≤7): Labs, **Systems**, Applications, Technology, Research, Trust, Investors (Purpose remains via footer Mission).
