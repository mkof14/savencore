...
type DiagramFigureProps = {
  ...
  sizes?: string;
  /** Same-page section anchor (D-0251). */
  href?: string;
  /** Accessible name for the diagram hit-link. */
  linkLabel?: string;
  children?: ReactNode;
};

/** Themeable chrome around owner-grade illustration assets (D-0234 / D-0241 / D-0251). */
function DiagramFigure({
  ...
  return (
    ...
    >
      {href && linkLabel ? (
        <a className="bmc-figure__hit" href={href} aria-label={linkLabel}>
          {frame}
        </a>
      ) : (
        frame
...
type DiagramNavProps = {
  href: string;
  linkLabel: string;
};

type LivingModelVisualProps = DiagramNavProps & {
  visualLabel: string;
  caption: string;
...
type LayerStackVisualProps = DiagramNavProps & {
  visualLabel: string;
  caption: string;
...
type DualRolesVisualProps = DiagramNavProps & {
  visualLabel: string;
  caption: string;
...
type EngineVisualProps = DiagramNavProps & {
  visualLabel: string;
  caption: string;
...
type SecondOpinionVisualProps = DiagramNavProps & {
  visualLabel: string;
  caption: string;
  signalLabel: string;
  resultLabel: string;
...
type BlackBoxVisualProps = DiagramNavProps & {
  visualLabel: string;
  caption: string;
  sides: readonly BioMathCoreBlackBoxSide[];
};

export function BlackBoxVisual({
  visualLabel,
  caption,
  sides,
  href,
...
type OutputVisualProps = DiagramNavProps & {
  visualLabel: string;
  caption: string;
...
type FormulaVisualProps = DiagramNavProps & {
  visualLabel: string;
  caption: string;
...
type EnvironmentsVisualProps = DiagramNavProps & {
  visualLabel: string;
  caption: string;
...
type SequenceVisualProps = DiagramNavProps & {
  visualLabel: string;
...
export function SequenceVisual({
  ...
  return (
    <div className="bmc-viz bmc-viz--sequence">
      <a className="bmc-figure__hit bmc-sequence__hit" href={href} aria-label={linkLabel}>
        <div className="bmc-sequence" role="group" aria-label={visualLabel}>
