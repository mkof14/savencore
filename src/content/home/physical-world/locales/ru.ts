...
export const physicalWorldHomeRu: PhysicalWorldHomeContent = {
  brand: "SAVEN Core",
  heroLine: "Интеллект для роботов, которые помогают людям",
  oneBreath:
    "SAVEN разрабатывает интеллектуальные технологии человеческой помощи для робототехники — связывая восприятие, движение, персонализацию, безопасность и взаимодействие с человеком, чтобы робототехнические системы поддерживали людей в физическом мире.",
  builds: ["Искусственный интеллект", "Робототехника", "Человеческая помощь"],
  ...
  clarity: {
    ...
    biomathBridge: {
      ...
      body: "Роботизированная помощь становится полезнее, когда технология может понять человека — а не только задачу. Human Data Model остаётся более широким структурированным представлением человеческих данных. BioMath Core может давать специализированный анализ и отчёты, которые SAVEN может использовать для более персонализированного взаимодействия — при разрешениях и контроле конфиденциальности. BioMath Core не заменяет Human Data Model.",
      scopeLine: "Охват модели: 20 категорий · 200+ сервисов — не каталог Operational.",
