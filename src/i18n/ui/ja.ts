...
export const uiJa: UiMessages = {
  ...
  lab: {
    ...
    videoEffectsApplied:
      "この帯に適用（ページ背景に溶け込む・柔らかいフェザー縁、硬い動画枠や額縁なし）：軽量デュアル形式の動画1本（WebM + MP4、モバイル選択）、レターボックス付きの最初のシーンを飛ばす再カット（約7.2秒・1.05×、中央ズームクロップ約1.14×、キャッシュ ?v=d0272）、任意の音声（開始時はミュート）、Mute 操作、控えめなテキストオーバーレイ、公開ページへの glass-light リンク、柔らかい進行バー上のチャプター印、スクロール連動のキャプション、強いスクロールパララックス（±約64px＋スケール）、弱めたデスクトップ・カーソルライト、Ken Burns 約1.06→1.14、Lab背景へ馴染む軽い側面アンビエント — グレイン・強いビネット・濁ったグレードなし。直角コーナー、ネオンなし。動きを抑える設定ではモーションを弱め、ポスターを表示。",
    videoEffectsIdeas:
      "将来のアイデア（未実装）：複数クリップ再生、スクラブ連動パララックス、チャプター見出し、広い対応時の HDR/AV1。",
