...
export const uiJa: UiMessages = {
  ...
  lab: {
    ...
    videoEffectsApplied:
      "この帯に適用（ページ背景に溶け込む・柔らかいフェザー縁、硬い動画枠や額縁なし）：軽量デュアル形式の動画1本（WebM + MP4、モバイル選択）、レターボックス付きの最初のシーンを飛ばす再カット（約7.6秒ループ、元の16:9、キャッシュ ?v=d0275）、任意の音声（開始時はミュート）、Mute 操作、控えめなテキストオーバーレイ、公開ページへの glass-light リンク、シーン時刻へジャンプするクリック可能なチャプター印、理解 → 支援 → ケアの時間連動キャプション、スクロール連動のキャプション、軽いスクロールパララックス、弱めたデスクトップ・カーソルライト、穏やかな Ken Burns、Lab背景へ馴染む軽い側面アンビエント — グレイン・強いビネット・濁ったグレードなし。直角コーナー、ネオンなし。動きを抑える設定ではモーションを弱め、ポスターを表示。",
    videoEffectsIdeas:
