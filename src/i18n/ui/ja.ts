...
export const uiJa: UiMessages = {
  ...
  lab: {
    ...
    videoEffectsApplied:
      "この帯に適用（フルブリード、額縁なし）：軽量デュアル形式の動画1本（WebM + MP4、モバイル選択）、再カット（約7.2秒・やや1.05×）、任意の音声（開始時はミュート）、Mute 操作、動画下の細いキャプション、ループ進行、強いスクロールパララックス（±約64px＋スケール）、はっきりしたデスクトップ・カーソルスポットライト、Ken Burns ズーム約1.12、はっきりしたビネット、グレイン、カラーグレード、フェードイン。直角コーナー、ネオンなし。動きを抑える設定ではモーションを弱め、ポスターを表示。",
    videoEffectsIdeas:
